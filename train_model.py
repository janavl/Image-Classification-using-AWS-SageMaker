...
def train(model, train_loader, criterion, optimizer, epochs, hook, device):
    '''
...
def main(args):
    ...
    model=train(cnn_model, train_loader, loss_criterion, optimizer, args.epochs, hook, device)
    
    '''
    TODO: Save the trained model
    '''
    logger.info("Save model to S3")
    model_path = os.path.join(args.model_dir, "cnn_model.pth")
    torch.save(model, model_path)
    
    '''
    TODO: Test the model to see its accuracy
    '''
    test(cnn_model, test_loader, loss_criterion, device, hook)
